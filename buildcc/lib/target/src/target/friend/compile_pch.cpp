...
void CompilePch::CacheCompileCommand() {
  source_path_ = ConstructSourcePath(target_.GetState().contains_cpp_src);
...
void CompilePch::BuildCompile() {
  ...
  if (target_.dirty_) {
    ...
    if (!fs::exists(source_path_)) {
      const std::string p =
          internal::Path::CreateNewPath(source_path_).GetPathAsString();
      env::SaveFile(p.c_str(), {"//Generated by BuildCC"}, false);
    }
